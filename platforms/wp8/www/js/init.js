...
$(function () 
{
    ...
    $.ajaxSetup({
        statusCode: {
            401: function () {
                document.location.href = '/login/?return=' + document.location.pathname;
            }
        }
    });
...
function navbarPos(event) 
{
    if (window.navbarScrolling === false) 
    {
        $('nav').css({"top":$(window).scrollTop()}).show();
    }
    window.navbarScrolling = false;
};

function navbarPosStart(event) 
{
    if (window.navbarScrolling !== true) 
    {
        $('nav').hide();
    }
    window.navbarScrolling = true;
    //console.log("Started...");
    //$('nav').hide().css({"top":$(window).scrollTop()}).show();
...
function timer() 
{
    ...
    $.ajax({
        type: "GET",
        url: "/index.php/api/notification/total",
        cache: false,
    ...
    $.ajax({
        type: "GET",
        url: "/index.php/api/messages/total",
        cache: false,
        success: function(data) {
            var obj = jQuery.parseJSON(data);
            if (typeof obj !== null) {
